...
function AdminProfile() {
  ...
  const addSocialLink = () => {
    setSocials((current) => [...current, { label: '', icon: '', url: '', display_order: current.length + 1 }]);
  };
  ...
  const addStat = () => {
    setStats((current) => [...current, { label: '', value: '', display_order: current.length + 1 }]);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const { coverPhotoUrl, ...profilePayload } = profile;
  ...
  return (
    <section className="admin-dashboard-section">
      ...
      {!loading && saving && <LoadingOverlay message="Saving profile changes..." />}
      <div className="admin-dashboard-header">
        <div>
          <h1>{isEditMode ? 'Edit Profile' : 'Profile Overview'}</h1>
          <p>Manage your main introduction, hero image, social links, roles, and portfolio stats.</p>
        </div>
        <div className="admin-header-actions">
          {isEditMode ? (
            ...
            <>
              <Link className="secondary-button" to="/profile/history">History</Link>
              <Link className="admin-link-button" to="/profile/edit">Edit profile</Link>
            </>
          )}
        </div>
      </div>
        ...
        <article className="admin-login-card admin-detail-card admin-profile-card">
          {profile.coverPhotoUrl && (
            <div className="admin-profile-hero">
              <img src={profile.coverPhotoUrl} alt="Hero cover" loading="lazy" />
              <div className="admin-profile-hero-overlay">
                <div>
                  ...
                  <p className="admin-profile-title">{profile.designation || 'Designation not set'}</p>
                </div>
              </div>
            </div>
          )}
                ...
                <div className="project-card-meta admin-profile-stat-list">
                  {stats.length > 0 ? (
                    stats.map((stat, index) => (
                      <span className="admin-chip" key={`${stat.label}-${index}`}>
      ...
      {isEditMode && (
        <div className="admin-login-card admin-form-card admin-profile-edit-card">
          {!loading && (
            <form onSubmit={handleSubmit} className="admin-form admin-profile-form">
              <div className="admin-form-header">
                <div>
                  <p className="admin-profile-form-tag">Profile settings</p>
                  <h2>Edit profile details</h2>
                  <p className="admin-form-description">Update your hero, contact info, socials, and resume stats from a polished dashboard view.</p>
                </div>
                <button type="submit" className="primary-button admin-form-save-button" disabled={saving}>
              ...
              <div className="admin-profile-form-grid">
                <section className="admin-profile-section-card">
                  ...
                  <label>
                    Name
                    <input type="text" value={profile.name} onChange={handleFieldChange('name')} required />
                    {validationErrors.name && <span className="admin-field-error">{validationErrors.name}</span>}
                  </label>
                  <label>
                    Designation
                    <input type="text" value={profile.designation} onChange={handleFieldChange('designation')} required />
                    {validationErrors.designation && <span className="admin-field-error">{validationErrors.designation}</span>}
                  </label>
                  <ImageUploadField
                ...
                <section className="admin-profile-section-card">
                  ...
                  <label>
                    Email
                    <input type="email" value={profile.email} onChange={handleFieldChange('email')} />
                    {validationErrors.email && <span className="admin-field-error">{validationErrors.email}</span>}
                  </label>
                  <label>
                    Phone
                    <input type="text" value={profile.phone} onChange={handleFieldChange('phone')} />
                  </label>
                  <label>
                    Location
                    <input type="text" value={profile.location} onChange={handleFieldChange('location')} />
                  </label>
                </section>
              </div>
              ...
              <section className="admin-profile-section-card admin-profile-section-card-full">
                ...
                <label>
                  About
                  <textarea rows="6" value={profile.about} onChange={handleFieldChange('about')} />
                  {validationErrors.about && <span className="admin-field-error">{validationErrors.about}</span>}
                </label>
              </section>
              ...
              <section className="admin-profile-section-card admin-profile-section-card-full">
                <div className="admin-array-header admin-array-header-spaced">
                  <div>
                    <h3>Typing roles</h3>
                    <p className="admin-form-description">Add the role titles shown in your hero section.</p>
                  </div>
                  <button type="button" className="secondary-button" onClick={addTypingRole}>
                    Add Role
                  </button>
                </div>
                {typingRoles.length === 0 ? (
                  ...
                  <div className="admin-array-grid">
                    {typingRoles.map((role, index) => (
                      <div key={`role-${index}`} className="admin-array-item admin-array-item-grid">
                        <input
                          type="text"
                          value={role}
                          ...
                          required
                        />
                        <button type="button" className="secondary-button secondary-button-sm" onClick={() => removeTypingRole(index)}>
                          Remove
                        </button>
                      </div>
                    ))}
              ...
              <section className="admin-profile-section-card admin-profile-section-card-full">
                <div className="admin-array-header admin-array-header-spaced">
                  <div>
                    <h3>Social links</h3>
                    <p className="admin-form-description">Add social channels for your portfolio profile.</p>
                  </div>
                  <button type="button" className="secondary-button" onClick={addSocialLink}>
                    Add Social Link
                  </button>
                </div>
                {socials.length === 0 ? (
                  <p className="admin-muted-text">No social links defined yet.</p>
                ) : (
                  <div className="admin-array-grid admin-array-grid-columns-2">
                    {socials.map((social, index) => (
                      <div key={`social-${index}`} className="admin-array-item admin-array-item-grid">
                        <input
                          type="text"
                          value={social.label}
                          onChange={handleListChange(setSocials, index, 'label')}
                          placeholder="Label"
                          required
                        />
                        <input
                        ...
                        <input
                          type="number"
                          min="1"
                          value={social.display_order ?? social.displayOrder ?? index + 1}
                          onChange={handleListChange(setSocials, index, 'display_order')}
                          aria-label={`Social link ${index + 1} display order`}
                        />
                        <button type="button" className="secondary-button secondary-button-sm" onClick={() => removeSocialLink(index)}>
                          Remove
                        </button>
                      </div>
                    ))}
              ...
              <section className="admin-profile-section-card admin-profile-section-card-full">
                <div className="admin-array-header admin-array-header-spaced">
                  <div>
                    <h3>Stats</h3>
                    <p className="admin-form-description">Highlight your achievements with portfolio stats.</p>
                  </div>
                ...
                {stats.length === 0 ? (
                  ...
                  <div className="admin-array-grid admin-array-grid-columns-2">
                    {stats.map((stat, index) => (
                      <div key={`stat-${index}`} className="admin-array-item admin-array-item-grid">
                        <input
                          type="text"
                          value={stat.label}
                        ...
                        <input
                          type="number"
                          min="1"
                          value={stat.display_order ?? stat.displayOrder ?? index + 1}
                          onChange={handleListChange(setStats, index, 'display_order')}
                          aria-label={`Portfolio stat ${index + 1} display order`}
                        />
                        <button type="button" className="secondary-button secondary-button-sm" onClick={() => removeStat(index)}>
                          Remove
                        </button>
                      </div>
                    ))}
              ...
              <div className="admin-form-actions-row">
                ...
                {validationErrors && Object.keys(validationErrors).length > 0 && (
                  <span className="admin-form-note">Please fix the highlighted fields before saving.</span>
                )}
              </div>
            </form>
          )}
