...
function ProjectDetails() {
  ...
  return <PageTransition><article className="project-detail page-section">
    <header className="case-hero"><Link className="text-link" to="/projects"><i className="bx bx-left-arrow-alt" /> Project archive</Link><span className="eyebrow">Case study · {String(index + 1).padStart(2, '0')}</span><h1>{project.title}</h1><p>{project.shortDescription}</p><div className="case-meta"><span><small>Role</small>{project.role}</span><span><small>Timeline</small>{project.duration}</span><span><small>Stack</small>{project.techStack.slice(0,3).join(', ')}</span></div></header>
