...
async function parseApiResponse(response) {
    ...
    if (!text) {
        throw new Error(response.statusText || 'Empty response from API');
    }
...
export async function fetchApiAuth(path, options = {}) {
    ...
    if (response.status === 401) {
        ...
        if (!retryResponse.ok) {
            ...
            throw new Error(retryPayload?.message || retryResponse.statusText || 'API request failed');
        }

        const retryPayload = await parseApiResponse(retryResponse);
        if (!retryPayload.success) {
            throw new Error(retryPayload.message || 'API request failed');
        }
